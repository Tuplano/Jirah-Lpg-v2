        ...
        <form onSubmit={handleSubmit} className="flex flex-col">
          <DialogHeader className="p-6 pb-2 space-y-1">
            <div className="flex items-center gap-2 text-primary">
              ...
              <DialogTitle className="text-xl">Record Refill</DialogTitle>
            </div>
            <DialogDescription className="text-sm text-muted-foreground/80">
              Send multiple LPG products in one refill batch. Inventory will update automatically.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid grid-cols-1 gap-6">
              <div className="space-y-2.5">
                ...
                <p className="text-[11px] text-muted-foreground/60 ml-1">
                  Adjust if this shipment was sent at a different time.
                </p>
              </div>
            </div>
                    ...
                    <div key={product.id} className="group flex flex-col sm:flex-row items-end gap-3 p-4 rounded-lg bg-muted/20 border border-border/40 hover:border-border/80 transition-all duration-200">
                      ...
                      <div className="w-full sm:w-28 space-y-1.5">
                        ...
                        <Input
                          ...
                          min="1"
                          max={availableEmpty}
                          className={`h-9 border-border/60 bg-background shadow-xs transition-shadow focus:ring-1 text-center font-medium ${exceedsAvailable ? "border-destructive text-destructive" : ""}`}
            ...
            <div className="rounded-xl border border-primary/20 bg-primary/5 p-5 mt-4">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                <div className="space-y-1">
                  <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60">Total Batch Cost</p>
          ...
          <DialogFooter className="px-6 py-4 bg-muted/30 border-t border-border/40 gap-3">
            ...
            <Button type="submit" disabled={isPending || !canSubmitSend} className="h-10 px-8 font-bold gap-2 shadow-sm shadow-primary/20">
              {isPending ? (
                 <div className="h-4 w-4 rounded-full border-2 border-white border-t-transparent animate-spin" />
              ) : (
                <RefreshCcw className="h-4 w-4" />
              )}
              {isPending ? "Processing..." : "Record Refill Shipment"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
